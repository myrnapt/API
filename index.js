...
var CONNECTION_STRING = "mongodb+srv://admin:3aNVj7bYnY2yqFb0@cluster0.cl3aycg.mongodb.net/?retryWrites=true&w=majority";
...
app.get('/api/mercados-mediavales/get-mercados',(request,response) => {
    database.collection('mercados-mediavales-collection').find({}).toArray((error,result) => {
        response.send(result);
    })
})


app.post('/api/mercados-mediavales/add-mercados', multer().none(),(request,response) => {
    database.collection('mercados-mediavales-collection').count({}, function(error, numOfDocs){
